...
void LORA_SSI_SETUP()
{
	...
	HWREG(PRCM_BASE + PRCM_O_SECDMACLKGR)=0x00000100;
	HWREG(PRCM_BASE + PRCM_O_SECDMACLKGS)=0x00000100;
	HWREG(PRCM_BASE + PRCM_O_SECDMACLKGDS)=0x00000100;
	PRCMLoadSet();
	while (!PRCMLoadGet())
	{
	}
	HWREG(PRCM_BASE + PRCM_O_PDCTL0SERIAL) |= 0x00000001;
	
	//GPIO_setOutputEnableDio(IOID_23, GPIO_OUTPUT_ENABLE);
	// GPIO_setConfig(Board_GPIO_LED1, GPIO_CFG_OUT_STD | GPIO_CFG_OUT_LOW);
	IOCPortConfigureSet(IOID_7, IOC_PORT_GPIO, IOC_IOMODE_NORMAL);
	IOCPortConfigureSet(IOID_12, IOC_PORT_GPIO, IOC_IOMODE_NORMAL);

	IOCPinTypeSsiMaster(SSI0_BASE, IOID_8,
	IOID_9, IOID_11,
	IOID_10);
	IOCPortConfigureSet(IOID_11, IOC_PORT_GPIO, IOC_IOMODE_NORMAL);


	DOSET |=  (0x00001E80);//setting 9, 10,11,12 as output 
	...
	DSCLR =0x00001000;//setting pin 12 to low
	
	//  __delay_cycles(240000);
	
	DOUT|=0x00001000;//setting pin12 to high
	

	 while(LoRabegin(867E6)!=0x12)
	{
		//return  LoRabegin(867E6);	
	};
